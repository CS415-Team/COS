...
{
	header("location:deliverer_home.php");
}

if(isset($login))
{
  $sql=mysqli_query($con,"select * from tbl_deliverer where fld_email='$username' && fld_password='$pswd' ");
  if(mysqli_num_rows($sql))
  {
      $_SESSION['d_id']=$username;
      header('location:deliverer_home.php');	
  }
  else
  {
...
<head>
  <meta charset="UTF-8">
    <title>Meal Deliverer Login</title>
    <!--bootstrap files-->
...
<div id="content-wrap" style="padding-bottom: 1rem;"><!-- all other page content -->
  <div class="container justify-content-center" style=" position:relative; padding:40px; border:1px solid #0197A5; margin-top:150px; width:400px;">
      <ul class="nav nav-tabs nabbar_inverse" id="myTab" style="background:#0197A5;border-radius:10px 10px 10px 10px;" role="tablist">
        <li class="nav-item">
            <a class="nav-link active" id="home-tab" data-toggle="tab" href="#login" role="tab" aria-controls="home" aria-selected="true">Deliverer Login</a>
        </li>
